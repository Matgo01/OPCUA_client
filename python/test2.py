...
query = "SELECT * FROM OPCUA_configuration"
...
for row in rows:
    config_data = {
        'id': row[0],
        'addressOpcuaServer': row[1],
        'nodeConfigs': []  # Initialize nodeConfigs list
    }
    # Fetch nodeConfigs for current configuration
    node_query = "SELECT  name, frequency FROM opcua_configuration_node_configs; WHERE opcua_configuration_id = %s"
...
with open('opcua_configurations.json', 'w') as json_file:
